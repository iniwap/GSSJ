 ...
 * 十方汉鼎
 * @海眸1990
 * 测试 CentOS 6.3, Mono 3.4
 * 
 * 1. 修正路径问题，规范文件名称。适应Linux环境
 * 2. 解耦操作系统API依赖，替换繁简转换解决方案（以GB2312词库为基准，建立繁简转换库）
 * 3. 解析搜狗细胞词库。允许细胞词库加入词库。优化分词准确度
 * 
 * 
...
namespace PanGu
{
    ...
    //@"[０-９\d]+|[^ａ-ｚＡ-Ｚa-zA-Z0-9０-９\u4e00-\u9fa5]|[ａ-ｚＡ-Ｚa-zA-Z]+|[\u4e00-\u9fa5]+";
            ...
            while (cur != null)
            {
                ...
                switch (cur.Value.WordType)
                {
                    case WordType.SimplifiedChinese:
                        ...
                        {
                            string simplified = DotSTConvert.ToSimplified(cur.Value.Word);
                                
                                //Microsoft.VisualBasic.Strings.StrConv(cur.Value.Word, Microsoft.VisualBasic.VbStrConv.SimplifiedChinese, 0);
                        ...
                        while (curChsMatch != null)
                        {
                            ...
                            if (_Options.OutputSimplifiedTraditional)
                            {
                                if (_Options.TraditionalChineseEnabled)
                                {
                                    ...
                                    {
                                        newWord = DotSTConvert.ToTraditional(wi.Word);
                                    ...
                                    {
                                        newWord = DotSTConvert.ToSimplified(wi.Word);
        ...
        static private void LoadDictionary()
        {
            ...
            _WordDictionary.Load(dir + "Dict.bytes");
        ...
        //将资源文件copy到可读写目录，仅适用于ios
        static private void copyResourcesToDocumentsDir()
        {
            //复制字典文件到可读写目录
            string dir = Application.persistentDataPath + "/";

            if (System.IO.File.Exists(dir + "Dict.bytes"))
            {
                return;
            }

            TextAsset dict = (TextAsset)Resources.Load("Dict");
            System.IO.File.WriteAllBytes(dir + "Dict.bytes", dict.bytes);

            //复制其他资源文件到可读写目录
            TextAsset ChsDoubleName1 = (TextAsset)Resources.Load("ChsDoubleName1");
            System.IO.File.WriteAllText(dir+ "ChsDoubleName1.txt", ChsDoubleName1.text);

            TextAsset ChsDoubleName2 = (TextAsset)Resources.Load("ChsDoubleName2");
            System.IO.File.WriteAllText(dir + "ChsDoubleName2.txt", ChsDoubleName2.text);

            TextAsset ChsSingleName = (TextAsset)Resources.Load("ChsSingleName");
            System.IO.File.WriteAllText(dir + "ChsSingleName.txt", ChsSingleName.text);

            TextAsset Stopword = (TextAsset)Resources.Load("Stopword");
            System.IO.File.WriteAllText(dir + "Stopword.txt", Stopword.text);

            TextAsset Synonym = (TextAsset)Resources.Load("Synonym");
            System.IO.File.WriteAllText(dir + "Synonym.txt", Synonym.text);

            TextAsset Wildcard = (TextAsset)Resources.Load("Wildcard");
            System.IO.File.WriteAllText(dir + "Wildcard.txt", Wildcard.text);
        }
        ...
        private static void InitInfinitiveVerbTable()
        {
            ...
            string infStr = "beat,beat,beaten#become,became,become#begin,began,begun#bend,bent,bent#bet,bet,bet#bid,bid,bid#bite,bit,bitten#bleed,bled,bled#blow,blew,blown#break,broke,broken#breed,bred,bred#bring,brought,brought#broadcast,broadcast,broadcast#build,built,built#buy,bought,bought#catch,caught,caught#choose,chose,chosen#cling,clung,clung#come,came,come#cost,cost,cost#creep,crept,crept#cut,cut,cut#deal,dealt,dealt#dig,dug,dug#do,did,done#draw,drew,drawn#dream,dreamed,dreamt#drink,drank,drunk#drive,drove,driven#eat,ate,eaten#fall,fell,fallen#feed,fed,fed#feel,felt,felt#fight,fought,fought#find,found,found#flee,fled,fled#fly,flew,flown#forbid,forbade,forbidden#forget,forgot,forgotten#forgive,forgave,forgiven#freeze,froze,frozen#get,got,gotten#give,gave,given#go,went,gone#grow,grew,grown#hang,hung,hung#have,had,had#hear,heard,heard#hide,hid,hidden#hit,hit,hit#hold,held,held#hurt,hurt,hurt#keep,kept,kept#know,knew,known#lay,laid,laid#lead,led,led#leave,left,left#lend,lent,lent#bet,bet,bet#bid,bid,bid#broadcast,broadcast,broadcast#hit,hit,hit#hurt,hurt,hurt#let,let,let#put,put,put#quit,quit,quit#set,set,set#shut,shut,shut#spread,spread,spread#upset,upset,upset#have,had,had#make,made,made#hear,heard,heard#lay,laid,laid#pay,paid,paid#say,said,said#sell,sold,sold#tell,told,told#bend,bent,bent#build,built,built#creep,crept,crept#deal,dealt,dealt#dream,dreamed,dreamt#feel,felt,felt#keep,kept,kept#leave,left,left#lend,lent,lent#lose,lost,lost#mean,meant,meant#send,sent,sent#sleep,slept,slept#spend,spent,spent#sweep,swept,swept#bring,brought,brought#buy,bought,bought#catch,caught,caught#fight,fought,fought#seek,sought,sought#teach,taught,taught#think,thought,thought#blow,blew,blown#do,did,done#draw,drew,drawn#go,went,gone#grow,grew,grown#know,knew,known#lie,lay,lain#see,saw,seen#shine,shined,shone#shine,shined,shone#let,let,let#lie,lay,lain#light,lit/lighted,lit/lighted#lose,lost,lost#make,made,made#mean,meant,meant#meet,met,met#pay,paid,paid#put,put,put#quit,quit,quit#read,read,read#ride,rode,ridden#ring,rang,rung#rise,rose,risen#run,ran,run#say,said,said#see,saw,seen#seek,sought,sought#sell,sold,sold#send,sent,sent#set,set,set#shake,shook,shaken#shine,shone,shone#shoot,shot,shot#show,showed,shown#shrink,shrank,shrunk#shut,shut,shut#sing,sang,sung#sink,sank,sunk#sit,sat,sat#sleep,slept,slept#slide,slid,slid#speak,spoke,spoken#speed,sped,sped#spend,spent,spent#spit,spat,spat#spread,spread,spread#stand,stood,stood#steal,stole,stolen#stick,stuck,stuck#strike,struck,struck#swear,swore,sworn#sweep,swept,swept#swim,swam,swum#swing,swung,swung#take,took,taken#teach,taught,taught#tear,tore,torn#tell,told,told#think,thought,thought#throw,threw,thrown#understand,understood,understood#upset,upset,upset#wake,woke,woken#wear,wore,worn#win,won,won#wind,wound,wound#withdraw,withdrew,withdrawn#write,wrote,written#show,showed,shown#swear,swore,sworn#throw,threw,thrown#wear,wore,worn#withdraw,withdrew,withdrawn#beat,beat,beaten#bite,bit,bitten#choose,chose,chosen#drive,drove,driven#eat,ate,eaten#fall,fell,fallen#forbid,forbade,forbidden#forget,forgot,forgotten#forgive,forgave,forgiven#freeze,froze,frozen#get,got,gotten#give,gave,given#hide,hid,hidden#ride,rode,ridden#rise,rose,risen#shake,shook,shaken#speak,spoke,spoken#steal,stole,stolen#take,took,taken#wake,woke,woken#write,wrote,written#bleed,bled,bled#breed,bred,bred#feed,fed,fed#flee,fled,fled#hold,held,held#lead,led,led#speed,sped,sped#cling,clung,clung#dig,dug,dug#drink,drank,drunk#hang,hung,hung#ring,rang,rung#run,ran,run#shrink,shrank,shrunk#sing,sang,sung#sink,sank,sunk#stick,stuck,stuck#strike,struck,struck#swim,swam,swum#swing,swung,swung#win,won,won#light,lit,lit#shoot,shot,shot#sit,sat,sat#slide,slid,slid#spit,spat,spat";
            string[] strs = infStr.Split('#');

            foreach (var inf in strs)
            {
                string[] ens = inf.Split(',');
                ...
                for (int i = 1; i < 3; i++)
                {
                    ...
                    {
                        _InfinitiveVerbTable.Add(key, ens[0]);
                    }
                }
            }
