...
Shader "Custom/Notebook"
{
    Properties
    {
        ...
        _LineAlpha ("Line Alpha", Range (0, 5)) = 3
        _GridWidth ("Grid Width", Range (1, 200)) = 80
        _GridSize ("Grid Size", Range (1, 1000)) = 100
        _AngleNum ("Angle Num", Range (1, 5)) = 3
        _SampNum ("Samp Num", Range (1, 16)) = 16
        _LineComplete ("Line Complete", Range (0.001, 1)) = 0.4
        _LineSoft ("Line Soft", Range (0.001, 1)) = 0.4

        _GridColor ("Grid Color", Color) = (0.25,0.1,0.1)
        _GridAlpha ("Grid Alpha", Vector) = (0.5,0.5,0,0)
        ...
        Pass
        {
            ...
            float4 fx(in float2 pos)
            {
                ...
                for (int i = 0; i< _AngleNum; i++)
                {
                    ...
                    {
                        float2 dpos = v.yx*float2(1, -1)*float(j)*Res.y / (1000 * _LineComplete);//[1,1000]
                        float2 dpos2 = v.xy*float(j*j) / float(_SampNum)*.5*Res.y / (1000*_LineSoft);//[1,1000]
                        float2 g;
                        float fact;
                        float fact2;
                ...
                col /= float(_SampNum*_AngleNum)*_LineAlpha / sqrt(Res.y);
                col2 /= sum;
                ...
                float2 s = sin(pos.xy*.1 / sqrt(Res.y / _GridSize));
                karo -= _GridColor*dot(exp(-s*s*_GridWidth), _GridAlpha);
                
                return float4(float3(col.x*col2*karo), 1);
            }
