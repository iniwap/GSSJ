...
namespace ImageEffects
{
    ...
    public class ColorGradingBase : ImageEffectBase
    {
        [Header("LUT")]
        ...
        protected Texture3D _internalLUT3D;
        protected Texture2D _internalLUT2D;
        ...
        void OnValidate()
        {
            if (userLUT)
            {
                ...
                {
                    if (_internalLUT2D != null)
                    {
                        DestroyImmediate(_internalLUT2D);
                    }

                    _internalLUT2D = userLUT;

                    if (_internalLUT3D != null)
                    {
                        DestroyImmediate(_internalLUT3D);
                    }
        ...
        public  static Texture3D LUT2DTo3D(Texture2D LUT)
        {
            int dim = LUT.height;
