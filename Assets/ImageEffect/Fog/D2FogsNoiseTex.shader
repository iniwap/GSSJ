...
Shader "Custom/D2FogsNoiseTex" {
    Properties{
        [HideInInspector]_Size("Size", float) = 0.02
		[HideInInspector]_Speed("Horizontal Speed", float) = 0.02
		[HideInInspector]_VSpeed("Vertical Speed", float) = 0
        [HideInInspector]_Density("Density", float) = 1.18
    ...
    Subshader{
        ...
        Pass{
            ...
            {
                float2 fogUV = float2 (i.uv.x, i.uv.y);
                float f = fog(fogUV);
